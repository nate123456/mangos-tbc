...
class CharacterHandler
{
    public:
        ...
        void HandlePlayerBotLoginCallback(QueryResult* /*dummy*/, SqlQueryHolder* holder)
        {
            ...
            botSession->HandlePlayerLogin(lqh, false); // will delete lqh
...
void PlayerbotMgr::LoginPlayerBot(ObjectGuid playerGuid, uint32 masterAccountId)
{
    ...
    LoginQueryHolder* holder = new LoginQueryHolder(masterAccountId, playerGuid);
    if (!holder->Initialize())
    {
        delete holder;                                      // delete all unprocessed queries
        return;
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder, const bool checkAccountMatch)
{
    ...
    if (checkAccountMatch && !pCurrChar->LoadFromDB(playerGuid, holder))
    {
